...
public class AccountMapper {
        ...
        try (Connection connection = connectionPool.getConnection()) {
            try (PreparedStatement ps = connection.prepareStatement(sql)) {
                ...
                if (rs.next()) {
                    ...
                    return new Account(accountID, email, password, isAdmin, balance);
                } else {
    ...
    public static void createAccount(String email, String password, app.persistence.ConnectionPool connectionPool) throws DatabaseException {
        ...
        try (Connection connection = connectionPool.getConnection()) {
            ...
            if (e.getMessage().startsWith("Fejl: Allerede oprettet. ")) {
                msg = "Email'en er allerede registreret. Prøv igen eller kontakt Via.";
            }
    ...
    public static List<Order> getAllOrdersByID(int accountID, app.persistence.ConnectionPool connectionPool) throws DatabaseException {
        ...
        String sql = "select * from order where account_id = ? order by order_id";
        ...
        try (Connection connection = connectionPool.getConnection()) {
            try (PreparedStatement ps = connection.prepareStatement(sql)) {
                ...
                while (rs.next()) {
                    ...
                    accountList.add(new Account(accountID, email, password, isAdmin, balance));
